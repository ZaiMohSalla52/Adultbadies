  ...
  <ProfileMediaFrame className={`vg-roster-media-frame vg-roster-media-frame-${mode}`}>
    ...
    {!imageUrl ? (
      <div className="vg-roster-image-empty" role="status" aria-live="polite">
        <p className="my-0">{status === 'generating' ? 'Identity portrait is generating' : status === 'failed' ? 'Portrait generation failed' : 'Portrait currently unavailable'}</p>
        <span>
          {status === 'generating'
            ? 'Locked reference in progress'
            : status === 'failed'
              ? 'Use profile actions to recover'
              : 'Using fallback visual state'}
        </span>
      </div>
    ) : null}
  </ProfileMediaFrame>
